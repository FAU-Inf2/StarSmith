...
class NumericValueExpression {
  ...
  nvetfunc ("#{(NumericType:getMathFunctionName nve.constructedType term.constructedType pm.symbol)}(${nve : NumericValueExpression}, ${term : Term})${pm : HiddenPLUSMINUS}") {
    this.constructedType = (NumericType:getBiggerType nve.constructedType term.constructedType);
    this.isUnique = false; # if the default value is returned, it is not unique anymore!
...
class Term {
  ...
  tmuldivfac_func ("#{(NumericType:getMathFunctionName term.constructedType factor.constructedType md.symbol)}(${term : Term}, ${factor : Factor})${md : HiddenMULDIV}") {
